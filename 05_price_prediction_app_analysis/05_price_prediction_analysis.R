...
separate_bike_description <- function(data, keep_description_column = TRUE,
                                      append = TRUE){
    ...
    output_tbl <- data %>% separate(
        description,
        sep = " - ",
        into = c("category_1", "categpry_2","frame_material"),
        remove = FALSE
    )
...
separate_bike_model <- function(data, keep_model_column = TRUE,
                                      append = TRUE){
    ...
    if (!keep_model_column) output_tbl <- output_tbl %>%
        select(-model)
